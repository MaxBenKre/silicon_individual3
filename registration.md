...
- **0.2 Plain-language summary ★** — We use an LLM to predict a distribution of possible responses for each persona and survey item. We sample directly from those normalized, uncalibrated probabilities and draw slider values uniformly within the selected response bins. This is the raw probability-run output: it applies neither the temperature calibration nor the response heaping used by our other entries.
- More specific: We utilize [QSTN](https://github.com/dess-mannheim/QSTN) to create battery prompts and verbalized distribution outcomes in five bins, while adding the questions and interventions verbatim to the prompt. Personas are given in interview format. After selecting a response bin directly from the model probabilities, an integer is drawn uniformly from the values in that bin. Donation and newsletter outcomes use their native discrete options. Ideally, we would let the LLM predict the probability of every exact slider value, but eliciting roughly 100 probabilities for every slider question is not computationally feasible in our setup.
- **0.3 Submission tier & approach family ★** — Tier 1; per-respondent simulation, single model, zero-shot.
- **0.4 Pipeline diagram** — Personas → verbatim condition text and full questionnaire → QSTN battery prompt → verbalized distribution → validation and fallback resolution → seeded direct bin selection → uniform within-bin sampling → recoding and composite construction → output CSV.
...
- **B.2 Access & context mode** — Local vLLM inference with a stateless chat-style prompt. Final production run was launched locally on 2026-08-11.
- **B.3 Configuration** — QSTN passes vLLM SamplingParams with temperature=1.0, min_p=0.0, presence_penalty=0.0, frequency_penalty=0.0, repetition_penalty=1.0, no user-supplied stop strings, ignore_eos=false, and min_tokens=0. vLLM automatically loads the model generation configuration, which sets top_p=0.95, top_k=20, bos_token_id=248044, eos_token_id=[248046, 248044], and pad_token_id=248044. bfloat16; tensor parallel size 2; GPU-memory utilization 0.95; maximum concurrent sequences 100; eager execution; custom all-reduce disabled; thinking enabled with <think>/</think> boundaries; maximum model and generation tokens 15,000. One complete battery completion is requested per persona. The vLLM engine seed recorded in logs is 0; QSTN uses its default base seed 42 to generate one deterministic sampling seed per persona; post-processing uses NumPy seed 20260804 for both response-bin selection and uniform within-bin sampling.
...
- **F.1 Runs & seeds** — Exactly one run per persona and all answers. vLLM is initialized with engine seed 0; QSTN uses its default base seed 42 to deterministically generate a distinct request seed for each persona. Post-processing uses one NumPy generator seeded with 20260804 for response-bin selection and uniform sampling within selected slider bins. Identical valid probability records and these settings reproduce the sampling.
- **F.2 Aggregation rule** — Qwen reports a probability for each possible response bin. After validation, nonzero vectors are normalized and sampled directly without temperature scaling or any other softmax transformation. One response bin is drawn per persona and item; for 0–100 slider items, an integer is then drawn uniformly from the selected inclusive bin. Donation and newsletter responses use their native discrete values. There is one model completion per persona and no averaging across completions.

## G · Validation & post-processing

- **G.1 Human validation** — N/A
- **G.2 Post-processing** — Parse JSON; attempt JSON repair if direct parsing fails; require expected item and bin keys and finite non-negative probabilities; normalize nonzero vectors that do not sum to one. Missing or invalid item distributions fall back in order to the condition mean, global mean, then uniform distribution. The normalized model probabilities are sampled directly, and slider values are drawn uniformly within selected bins. No persona is excluded; the output contains 18,000 rows across 17 conditions. Raw and parsed records retain validation metadata.
- **G.3 Calibration corrections** — N/A. This entry applies no CCAM-derived calibration, temperature scaling, demographic adjustment, or response heaping. Model probability vectors are used at T = 1 after normalization.
...
- **I.2 External human data †** — Census Bureau 2024 ACS 1-Year and CPS ASEC tables, and the Pew Research Center 2026 National Public Opinion Reference Survey party-affiliation fact sheet, were used to construct demographic constraints documented in the source code repository. Climate Change in the American Mind data were used elsewhere in the broader design process to evaluate calibration variants, but no CCAM-derived calibration is applied to this entry. No human outcome data from the benchmark study were accessed.
...
- **J.1 Design-space search †** — The same probability-prediction approach was evaluated on 20 Climate Change in the American Mind questions across ten waves from 2020–2024 as part of the broader design process. This secondary entry intentionally preserves the raw T = 1 probability-run output as an ablation: it applies neither calibration nor within-bin heaping. No benchmark outcome data informed this choice. This is our purest LLM prediction, without informed additional input.

## K · Reproducibility & frozen artifacts

- **K.1 Code & materials** — https://github.com/dess-mannheim/silicon_sampling_benchmark; relevant files include the probability setup, prompt, configuration, persona manifest, launcher, and result-saving code. This deposit contains predictions/team_6_T1_secondary-2_v1.csv. The source repository records the post-processing seeds. The code DOI can be found in metadata.json after the Zenodo upload.
...
- **K.3 Computational resources** — Final probability production run 6830 used local two-GPU vLLM inference and no API calls. Elapsed wall-clock time was 33 h 58 m 52 s from run launch (2026-08-11 08:49:14) to writing the raw, parsed, and submitted T = 1 prediction file (2026-08-12 18:48:06); the QSTN battery itself completed after 33 h 54 m 31 s. The submitted file was produced as part of the original result-saving step and required no later calibration or heaping pass. A representative rendered QSTN battery prompt tokenizes to 2,957 Qwen tokens, implying approximately 53,226,000 input tokens for 18,000 personas; this is an approximation because condition-text lengths vary. At the 15,000-token generation limit, the output-token upper bound is 270,000,000.
